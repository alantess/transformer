...
class ReplayBuffer():
    def __init__(self, capacity, input_dims, n_actions,embed_len):
        ...
        self.mem_cntr = 0
        self.state_memory = np.zeros((capacity, input_dims,embed_len), dtype=np.float32)
        self.action_memory = np.zeros((capacity, n_actions), dtype=np.int64)
        self.reward_memory = np.zeros(capacity, dtype=np.float32)
        self.new_state_memory = np.zeros((capacity, input_dims, embed_len), dtype=np.float32)
